...
#include<ctype.h>
#include<string.h>
//下面定义保留字表，为简化程序，使用字符指针数组保存所有保留字 //如果想增加保留字,可继-添加，并修改保留字数目keywordSum 
#define keywordSum 8
...
//用于接收输入输出文件名，在tesr_main.c中定义
extern FILE * fin, * fout;	//用于指向输入输出文件的指针，在main.c工中定义
int TESTscan()	//词法分析函数
{
	char ch,token[40];	//ch为每次读入的字符，token用于保存识别出的单词
	int es=0,j,n;	    //es错误代码，0表本没有错误
